...
$gene.Mutate = function(){
  ...
  this.point = function(sequence, symbols){
    return sequence[this.ranAddr(sequence)] = symbols[this.ranAddr(symbols)];
  };
  ...
  this.sliceSeq = function(sequence, fragment){
    ...
    injectAddr = this.ranAddr(sequence);
  ...
  this.fragment = function(sequence){
    return sequence.substring(this.ranAddr(sequence),this.ranAddr(sequence));
  };
  ...
  this.getEndcap = function(sequence){
    return slice(0, this.ranAddr(sequence));
  };

  // given an array or string, returns a random address.
  this.ranAddr = function(sequence){
...
$gene.codex = function(symbol, func){
  gene.bases[symbol] = func;
};
